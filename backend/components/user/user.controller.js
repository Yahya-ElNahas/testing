...
const { auth, decryptToken } = require('../../auth');
...
module.exports.Init_User = (app) => {
    ...
    app.get('/user/cart/all', auth('customer'), async (req, res) => {
        try {
            const id = decryptToken(req).id;
            const user = await UserService.getUserCartById(id);
            res.status(200).json(user);
        } catch (error) {
            res.status(404).json({ error: error.message });
        }
    ...
    app.patch('/user/addTo/cart', auth('customer'), async (req, res) => {
        try {
            const id = decryptToken(req).id;
            const cart = await UserService.addToCart(id, req.body.item_id);
            res.status(200).json({ message: 'Item added to cart', cart });
        } catch (error) {
            res.status(400).json({ error: error.message });
        }
    ...
    app.post('/user/login', async (req, res) => {
        try {
            ...
            // Return token, user, and message in response
